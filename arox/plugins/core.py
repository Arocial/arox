...
class CorePlugin(Plugin):
    ...
    async def model_command(self, name: str, arg: str):
        if not arg:
            await self.agent.io_channel.agent_send("Please specify a model name")
            return
        self.agent.set_model(arg)
        await self.agent.io_channel.agent_send(f"Model switched to {arg}")
    ...
    async def info_command(self, name: str, arg: str):
        ...
        await self.agent.io_channel.agent_send(f"Current model: {current_model}")

        for provider in self.agent.get_capability(AGENT_INFO):
            info = await provider()
            if info:
                await self.agent.io_channel.agent_send(info)
    ...
    async def reset_command(self, name: str, arg: str):
        ...
        for provider in self.agent.get_capability(AGENT_RESET):
            provider()
        await self.agent.io_channel.agent_send("Reset complete.")

    @command("agent", "Call a subagent - /agent <name> [task]")
    async def agent_command(self, name: str, arg: str):
        parts = arg.split(maxsplit=1)
        if not parts:
            await self.agent.io_channel.agent_send("Usage: /agent <name> [task]")
            return
        ...
        if not subagent:
            await self.agent.io_channel.agent_send(
        ...
        if hasattr(subagent, "handle_task"):
            ...
            if result:
                await self.agent.io_channel.agent_send(result)
        else:
            await self.agent.io_channel.agent_send(
