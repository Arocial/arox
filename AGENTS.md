...
- **Per-agent channel** (`arox/core/io.py`): every runtime holds its own `agent_ep: AgentIOEndpoint` bound to its session and uses `send` / `receive` to talk to the UI. The endpoint automatically maintains a journal cursor plus replay events. A safe boundary is recorded only when a turn or command finishes and no turn or accepted command remains active. At the replay threshold (default 1000 events) or on reconnect, the completed prefix is replaced by that cursor; in-flight events, unjournaled output, and latest state events are retained. Reconnect sends `SnapshotEvent(journal_id)` followed by retained events. `None` means empty history. Adapters reconstruct snapshots through the given journal ID, inclusively, using `build_io_timeline(through_id=...)`; they must not read the latest journal for an older snapshot. Snapshot compaction does not alter an existing consumer's inbox and is not a hard memory bound during a long turn. Runtime and plugins never explicitly snapshot. Adapter-originated events are dispatched to registered handlers by the agent endpoint's receive loop. Both the main runtime and each subagent runtime have independent channels, so their output can be routed/rendered separately.
