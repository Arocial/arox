...
- **`AgentSession`** is the persistent, authoritative entity tracking:
  ...
  - Message history is persisted as segments on the session and is the runtime source of truth for LLM context. Session events form the UI timeline: started `ClientInput` events and completed command events carry their normalized input, step events reference stable IDs stored in `ModelMessage.metadata`, and compaction events expose context-boundary metadata without exposing summaries. `build_io_snapshot()` projects model and command events together with active and archived message segments, so commands are visible after resume without entering the model context. User turns are located through the `server_message_id` carried by `ClientInput` and attached to message content metadata. Compaction archives the processor's original messages, including the current user request, so historical forks can locate and slice the appropriate messages without replaying events or duplicating model payloads.
